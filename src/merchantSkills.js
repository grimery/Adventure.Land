...
let minCompoundScrolls = 10;
let upgradeableItems = ["hpamulet", "hpbelt", "ringsj"];
let trashType = ["material", "weapon", "shoes", "helmet"];
let mPotionThreshold = 30;
let hPotionThreshold = 1;
let sellItemLevel = 2;
let profitMargin = 1.6;
let manaReserve = 0.8;

function merchantSkills(farmTarget){
			...
			smart_move({to:"scrolls"}, () => {
				...
				smart_move({to:"exchange"}, () => {
					...
					smart_move({to:"bank"}, () => {
						...
						smart_move({to:"town"}, () => {
							smart_move({x: character.x  - 55, y: character.y + 60}, () => {
								//parent.socket.emit("merchant",{num:41});
								parent.open_merchant(41);
								sellItems(sellItemLevel, profitMargin);
...
function selLTrash(){
	for(let i = 0; i <= 34; i++){
		if(character.items[i] && trashType.indexOf(G.items[character.items[i].name].type) !== -1) {
